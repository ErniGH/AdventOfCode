...
def recorrerLaberinto(actual, lista_visitados, caminos, segundoIntento):
    total = 0
    if actual == "end":
        total += 1
    else:
        for c in caminos[actual]:
            if c not in lista_visitados or segundoIntento:
                fs = True
                if c in lista_visitados:
                    fs = False
                visitadosNueva = lista_visitados.copy()
                if c.islower():
                    visitadosNueva.append(c)
                total += recorrerLaberinto(c, visitadosNueva, caminos, segundoIntento and fs)
    return total

lista = []
lista_visitados = []
total = recorrerLaberinto("start", lista_visitados, caminos, True)
